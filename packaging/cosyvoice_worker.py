...
def _patch_monotonic_align() -> None:
    """Inject pure-Python maximum_path_c if the Cython extension is missing."""
    try:
        import matcha.utils.monotonic_align.core  # noqa: F401
        return  # Cython extension compiled — nothing to do
    ...
    def maximum_path_c(paths, values, t_xs, t_ys, max_neg_val=-1e9):
        ...
        for i in range(b):
            _maximum_path_each(
                paths[i], values[i], int(t_xs[i]), int(t_ys[i]), max_neg_val
            )

    mod = types.ModuleType("matcha.utils.monotonic_align.core")
    mod.maximum_path_c = maximum_path_c  # type: ignore[attr-defined]
...
def main() -> int:
    ...
    if source.is_dir():
        ...
        if matcha.is_dir():
            sys.path.insert(1, str(matcha))
        os.environ["PYTHONPATH"] = os.pathsep.join(
            [str(source)] + ([str(matcha)] if matcha.is_dir() else []))
        _patch_monotonic_align()
