...
REWRITE_INSTRUCTION = (
    "分两步完成：\n\n"
    "**第一步 — 完整翻译**：先把下面的原文全文翻译成通顺的中文，"
    "确保不漏掉任何技术细节、数据、结论和引用。\n\n"
    "**第二步 — 自媒体转写**：在翻译的基础上，用[小红书/公众号]风格写成中文自媒体报道。\n"
    "要求：\n"
    "- **开头有钩子**：前几句要抓住读者注意力\n"
    "- **用小标题分段**：2-4 个小标题，逻辑清晰，读者可以扫读\n"
    "- **图文结合**：文中用 `[[IMG1]]`、`[[IMG2]]` 等占位符在合适位置标记插图\n"
    "- **通俗易懂**：用大白话解释复杂概念，避免术语堆砌\n"
    "- **口语化但不失专业**：像朋友聊天一样讲清楚一件事\n"
    "- **适合手机阅读**：短段落（不超过 4 行）、多用分点、重点加粗\n"
    "只能依据原文事实，不得编造。\n\n"
    "输出严格的 JSON，字段：\n"
    '  "title_candidates": [3 个吸睛但不虚假的标题],\n'
    '  "body_md": "Markdown 正文，小标题+短段落+插入 [[IMG1]] 等占位符"\n'
    "不要输出 JSON 以外的任何内容。\n\n"
    "原文标题：{title}\n来源：{source}\n\n原文正文：\n{content}"
)
